...
def _get_secrets_dict() -> dict:
    """Extract service account info from secrets.toml — supports both key formats."""
    # Try format 1: [gcp_service_account]
    if "gcp_service_account" in st.secrets:
        raw = st.secrets["gcp_service_account"]
    # Try format 2: [connections.gsheets]
    elif "connections" in st.secrets and "gsheets" in st.secrets["connections"]:
        raw = st.secrets["connections"]["gsheets"]
    else:
        raise KeyError(
            "لم يتم العثور على credentials في secrets.toml.\n"
            "أضف [gcp_service_account] أو [connections.gsheets] في الملف."
        )
    return {
        "type":                        str(raw.get("type", "service_account")),
        "project_id":                  str(raw["project_id"]),
        "private_key_id":              str(raw["private_key_id"]),
        "private_key":                 str(raw["private_key"]).replace("\\n", "\n"),
        "client_email":                str(raw["client_email"]),
        "client_id":                   str(raw["client_id"]),
        "auth_uri":                    str(raw.get("auth_uri",  "https://accounts.google.com/o/oauth2/auth")),
        "token_uri":                   str(raw.get("token_uri", "https://oauth2.googleapis.com/token")),
        "auth_provider_x509_cert_url": str(raw.get("auth_provider_x509_cert_url", "https://www.googleapis.com/oauth2/v1/certs")),
        "client_x509_cert_url":        str(raw.get("client_x509_cert_url", "")),
    }

@st.cache_resource(ttl=3600)
def _get_client():
    """Build and cache a gspread client from secrets.toml credentials."""
    info  = _get_secrets_dict()
...
def _check_connection() -> bool:
    ...
    if not GSHEETS_AVAILABLE:
        ...
        return False
    try:
        _get_secrets_dict()   # validates secrets exist & have required keys
    except KeyError as e:
        st.error(f"❌ secrets.toml: {e}")
        st.info(
            "📋 تأكد إن ملف `.streamlit/secrets.toml` يحتوي على أحد الصيغتين:\n\n"
            "`[gcp_service_account]`  أو  `[connections.gsheets]`\n\n"
            "مع الحقول: type, project_id, private_key_id, private_key, client_email, client_id"
        )
        return False
    try:
        _get_client()
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        if st.button("🔄 Refresh Data", key="clear_cache_btn", use_container_width=True):
            st.cache_data.clear()
            st.toast("✅ Cache cleared — data refreshed from Google Sheets", icon="🔄")
            st.rerun()
