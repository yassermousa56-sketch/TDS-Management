...
# ─────────────────────────────────────────────
_offline_data = pd.DataFrame(columns=[
...
_offline_payments = pd.DataFrame(columns=["Company Name", "Payment Date", "Amount", "Method", "Reference", "Notes"])
_is_offline = False
...
def page_security():
    page_header("🔐 Security & Access Control", f"{BRAND['name']} · manage users, passwords, and access permissions")
    users = load_users()
...
def _check_connection() -> bool:
    ...
    if not GSHEETS_AVAILABLE:
        st.warning("⚠️ gspread / google-auth غير مثبتين.")
        _is_offline = True
        return False
    if "gcp_service_account" not in st.secrets:
        st.warning("⚠️ مفيش `[gcp_service_account]` في secrets. العمل في وضع Offline.")
        _is_offline = True
        return False
    try:
        _get_client()
        ...
        st.warning(f"⚠️ فشل الاتصال بـ Google Sheets: {e}\\n**العمل في وضع Offline** - البيانات مؤقتة في الذاكرة.")
        _is_offline = True
        return False
...
def load_data() -> pd.DataFrame:
    global _offline_data, _is_offline
    if _is_offline:
        return _offline_data.copy()
    try:
        ...
        _offline_data = df.copy()
    except Exception as e:
        st.warning(f"⚠️ Could not load from Google Sheets: {e}. Using offline data.")
        return _offline_data.copy()
...
def save_data(df: pd.DataFrame):
    ...
    if _is_offline:
        st.info("💾 تم الحفظ في وضع Offline (الذاكرة المؤقتة). لن يتم حفظ البيانات في Google Sheets.")
        return
    ...
    try:
        ...
        st.error(f"❌ فشل الحفظ في Google Sheets: {e}")
        st.info("💡 البيانات محفوظة في الذاكرة مؤقتاً. حاول لاحقاً أو تحقق من إعدادات الاتصال.")
...
def load_payments() -> pd.DataFrame:
    global _offline_payments, _is_offline
    if _is_offline:
        return _offline_payments.copy()
    try:
        ...
        _offline_payments = pf.copy()
    except Exception as e:
        st.warning(f"⚠️ Could not load payments: {e}. Using offline data.")
        return _offline_payments.copy()
    
    pf = _offline_payments.copy()
...
def save_payments(pf: pd.DataFrame):
    ...
    try:
        ...
        st.error(f"❌ فشل حفظ المدفوعات: {e}")
...
def chart_exhibitors_by_category(df):
    if df.empty: return go.Figure()
    g = df["Category"].value_counts().reset_index()
...
def chart_payment_status(df):
    if df.empty: return go.Figure()
    g = df["Payment Status"].value_counts().reset_index()
...
def ai_insights(df: pd.DataFrame) -> list:
    ...
    ins = []
    total = df["Total Booth Price"].sum()
    col = df["Paid Amount"].sum()
    rem = df["Remaining Amount"].sum()
    rate = (col/total*100) if total > 0 else 0
    up = df[df["Payment Status"]=="Unpaid"]
...
def page_data_entry():
    ...
    if do_save:
        if not company.strip():
            ...
            st.error("❌ Booth Area must be > 0.")
        elif live_ppm <= 0:
            st.error("❌ Price per m² must be > 0.")
        else:
            rec = {
            ...
            if st.session_state.edit_mode and st.session_state.edit_idx is not None:
                ...
                with st.spinner("☁️ Saving..."):
                    add_record(rec)
                    if live_paid > 0:
                        log_payment(company.strip(), cdate, live_paid, pay_method_init, "", "Initial payment")
                st.success(f"✅ **{company}** saved! Total: EGP {total_price:,.0f}")
            st.rerun()
...
def page_analytics():
    ...
    df = load_data()
    if df.empty: 
        st.warning("No exhibitors registered yet.")
        return
...
def page_records():
    ...
    df = load_data()
    if df.empty: 
        st.warning("No records yet. Register exhibitors first.")
        return
    ...
    with tab_hist:
        pf = load_payments()
        cp = pf[pf["Company Name"]==selected].copy()
        c1, c2, c3 = st.columns(3)
            ...
            for _, pr in cp_s.iterrows():
                ...
                with col_b:
                    if st.button("🗑️", key=f"dp_{orig_idx}_{selected}", help="Delete payment"):
                        delete_payment_entry(orig_idx, selected)
    ...
    with tab_edit:
        ...
        with st.form(f"edit_{selected}", clear_on_submit=False):
            ...
            if do_delete:
                ...
                st.success(f"🗑️ Deleted {selected} and all payment records.")
                st.rerun()
...
def page_settings():
    ...
    with c3:
        rate = (df["Paid Amount"].sum()/df["Total Booth Price"].sum()*100) if df["Total Booth Price"].sum()>0 else 0
        st.markdown(
            kpi_card(BRAND["name"], f"{rate:.1f}% collected", f"{len(df)} exhibitors · {len(pf)} payments"),
            unsafe_allow_html=True,
        )
        ...
        try:
            ...
            if missing: 
                ...
                if st.button("✅ Confirm Import → Push to Google Sheet"):
                    ...
                    st.success(f"✅ {len(new_df)} records pushed to Google Sheet.")
                    st.rerun()
        except Exception as e: 
    ...
    with c3:
        ...
        st.markdown(
            ...
            f'Library (gspread): <b>{"✅ Installed" if gs_ok else "❌ Missing"}</b><br>'
    ...
    with c4:
        st.markdown(
            f'<div class="insight-card"><div class="insight-title">⚙️ Setup Guide</div>'
            f'<div class="insight-body" style="font-size:11px;">'
            f'1. Enable Google Sheets API & Drive API in Google Cloud<br>'
            f'2. Share Sheet with <b>client_email</b> as <b>Editor</b><br>'
            f'3. Add secrets in Streamlit Cloud Settings'
            f'</div></div>',
            unsafe_allow_html=True,
        )
...
def main():
    ...
    # Check connection but DON'T stop if failed - work in offline mode
    _check_connection()
